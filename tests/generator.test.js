...
// -----------------------------------------------------------------------
test('the real content-index.json passes full schema + cross-record validation (41 records)', () => {
  const manifest = gen.loadManifest(REAL_MANIFEST_PATH);
  assert.doesNotThrow(() => gen.validateManifest(manifest));
  assert.equal(manifest.records.length, 41);
});
...
test('MT-21: every /articles/ generated card shows section + reading time and no date element', () => {
  ...
  assert.equal(cards.length, 15, 'expected 15 article cards');
...
test('T-11: every include_in_articles post appears as a card in articles/index.html, and no extras', () => {
  ...
  assert.equal(expected.length, 15, 'expected 15 articles-eligible posts');
  const html = fs.readFileSync(path.join(ROOT, 'articles', 'index.html'), 'utf8');
  const region = html.match(/GENERATED:ARTICLES_LIST:START[\s\S]*?GENERATED:ARTICLES_LIST:END/)[0];
...
// -----------------------------------------------------------------------
test('T-15: search-index.json contains exactly the 28 searchable records and no hub/utility', () => {
  const manifest = gen.loadManifest(REAL_MANIFEST_PATH);
  gen.validateManifest(manifest);
  ...
  assert.equal(expected.length, 28, 'expected 28 searchable records');
...
test('T-28: feeds carry posts + data experiences; sitemap excludes 404/template and includes /search/; <lastmod> only when dated', () => {
  ...
  assert.equal(itemCount, 17, 'feed carries 15 posts + 2 data experiences');
  ...
  assert.equal(locCount, 39, 'sitemap has 39 entries (41 records − 404 − template)');
  assert.equal(lastmodCount, 17, '<lastmod> appears only on the 17 dated records');
});
