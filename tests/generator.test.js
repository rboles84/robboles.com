...
// -----------------------------------------------------------------------
test('the real content-index.json passes full schema + cross-record validation (35 records)', () => {
  const manifest = gen.loadManifest(REAL_MANIFEST_PATH);
...
test('MT-21: every /articles/ generated card shows section + reading time and no date element', () => {
  ...
  assert.equal(cards.length, 12, 'expected 12 article cards');
...
test('T-11: every include_in_articles post appears as a card in articles/index.html, and no extras', () => {
  ...
  assert.equal(expected.length, 12, 'expected 12 articles-eligible posts');
  const html = fs.readFileSync(path.join(ROOT, 'articles', 'index.html'), 'utf8');
  const region = html.match(/GENERATED:ARTICLES_LIST:START[\s\S]*?GENERATED:ARTICLES_LIST:END/)[0];
...
// -----------------------------------------------------------------------
test('T-15: search-index.json contains exactly the 22 searchable records and no hub/utility', () => {
  const manifest = gen.loadManifest(REAL_MANIFEST_PATH);
  gen.validateManifest(manifest);
  ...
  assert.equal(expected.length, 22, 'expected 22 searchable records');
...
test('T-28: feeds carry posts + data experiences; sitemap excludes 404/template and includes /search/; <lastmod> only when dated', () => {
  ...
  assert.equal(itemCount, 14, 'feed carries 12 posts + 2 data experiences');
  ...
  assert.equal(locCount, 33, 'sitemap has 33 entries (35 records − 404 − template)');
  assert.equal(lastmodCount, 14, '<lastmod> appears only on the 14 dated records');
});
