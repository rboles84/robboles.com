...
// -----------------------------------------------------------------------
test('the real content-index.json passes full schema + cross-record validation (43 records)', () => {
  const manifest = gen.loadManifest(REAL_MANIFEST_PATH);
  assert.doesNotThrow(() => gen.validateManifest(manifest));
  assert.equal(manifest.records.length, 43);
});
...
// -----------------------------------------------------------------------
test('T-15: search-index.json contains exactly the 30 searchable records and no hub/utility', () => {
  const manifest = gen.loadManifest(REAL_MANIFEST_PATH);
  gen.validateManifest(manifest);
  ...
  assert.equal(expected.length, 30, 'expected 30 searchable records');
...
test('T-28: feeds carry posts + data experiences; sitemap excludes 404/template and includes /search/; <lastmod> only when dated', () => {
  ...
  assert.equal(itemCount, 18, 'feed carries 16 posts + 2 data experiences');
  ...
  assert.equal(locCount, 41, 'sitemap has 41 entries (43 records − 404 − template)');
  assert.equal(lastmodCount, 18, '<lastmod> appears only on the 18 dated records');
});
