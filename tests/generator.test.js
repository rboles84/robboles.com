...
// -----------------------------------------------------------------------
test('the real content-index.json passes full schema + cross-record validation', () => {
  const manifest = gen.loadManifest(REAL_MANIFEST_PATH);
  assert.doesNotThrow(() => gen.validateManifest(manifest));
  assert.ok(manifest.records.length > 0, 'the real content inventory must not be empty');
});
...
test('MT-21: every /articles/ generated card shows section + reading time and no date element', () => {
  ...
  assert.equal(cards.length, 19, 'expected 19 article cards');
...
test('T-11: every include_in_articles post appears as a card in articles/index.html, and no extras', () => {
  ...
  assert.equal(expected.length, 19, 'expected 19 articles-eligible posts');
  const html = fs.readFileSync(path.join(ROOT, 'articles', 'index.html'), 'utf8');
  const region = html.match(/GENERATED:ARTICLES_LIST:START[\s\S]*?GENERATED:ARTICLES_LIST:END/)[0];
...
// -----------------------------------------------------------------------
test('T-15: search-index.json contains exactly the 35 searchable records and no hub/utility', () => {
  const manifest = gen.loadManifest(REAL_MANIFEST_PATH);
  gen.validateManifest(manifest);
  ...
  assert.equal(expected.length, 35, 'expected 35 searchable records');
...
test('T-28: feeds carry posts + data experiences; sitemap excludes 404/template and includes /search/; <lastmod> only when dated', () => {
  ...
  assert.equal(itemCount, 20, 'feed carries the newest 20 eligible records');
  ...
  assert.equal(locCount, 46, 'sitemap has 46 entries (48 records − 404 − template)');
  assert.equal(lastmodCount, 23, '<lastmod> appears only on the 23 dated records');
});
